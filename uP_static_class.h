...
public:
    ...
    template <typename... Args, typename = decltype(TDerived(std::declval<Args>()...))>
    ...
    {
        DEBUG_MSG("New static class instantiated at address: " << _s_class_ptr << std::endl);
    }
    ...
    {
        DEBUG_MSG("Copy constructor called, new static class instantiated"
                  " at address: "
    ...
    static_class_t<TDerived, MaxInstances>& operator=(const static_class_wrapper& other) = default;

    /* Move assignment operator from static class */

    static_class_t<TDerived, MaxInstances>&
    operator=(static_class_t<TDerived, MaxInstances>&& other) noexcept
    {
    ...
    ~static_class_wrapper()
    {
        if (_s_class_ptr) {
            _s_class_ptr->~_class_ptr_s();
            allocator<_class_ptr_s<TDerived> >()->deallocate(_s_class_ptr);
            DEBUG_MSG("Static class de-allocated from address: " << _s_class_ptr << std::endl);
    ...
    operator TDerived*()
    {
        return _s_class_ptr->_class_ptr;
    }
